...
SDLPadInputDriver::SDLPadInputDriver(xe::ui::Window* window)
    : InputDriver(window),
...
SDLPadInputDriver::~SDLPadInputDriver() {
...
X_STATUS SDLPadInputDriver::Setup() {
  if (!TestSDLVersion()) {
  ...
  window_->loop()->PostSynchronous([&]() {
    ...
    SDL_EventFilter event_filter{[](void* userdata, SDL_Event* event) -> int {
      ...
      const auto driver = static_cast<SDLPadInputDriver*>(userdata);
...
X_RESULT SDLPadInputDriver::GetCapabilities(uint32_t user_index, uint32_t flags,
...
X_RESULT SDLPadInputDriver::GetState(uint32_t user_index,
...
X_RESULT SDLPadInputDriver::SetState(uint32_t user_index,
...
X_RESULT SDLPadInputDriver::GetKeystroke(uint32_t user_index, uint32_t flags,
                                         X_INPUT_KEYSTROKE* out_keystroke) {
  // TODO(joellinn) translate keyboard events for chatpad emulation.
  return X_ERROR_EMPTY;
}

void SDLPadInputDriver::OnControllerDeviceAdded(SDL_Event* event) {
...
void SDLPadInputDriver::OnControllerDeviceRemoved(SDL_Event* event) {
  assert(window_->loop()->is_on_loop_thread());
  std::unique_lock<std::mutex> guard(controllers_mutex_);
...
void SDLPadInputDriver::OnControllerDeviceAxisMotion(SDL_Event* event) {
  assert(window_->loop()->is_on_loop_thread());
  std::unique_lock<std::mutex> guard(controllers_mutex_);
...
void SDLPadInputDriver::OnControllerDeviceButtonChanged(SDL_Event* event) {
  assert(window_->loop()->is_on_loop_thread());
  std::unique_lock<std::mutex> guard(controllers_mutex_);
...
std::tuple<bool, size_t> SDLPadInputDriver::GetControllerIndexFromInstanceID(
...
SDLPadInputDriver::ControllerState* SDLPadInputDriver::GetControllerState(
    uint32_t user_index) {
...
bool SDLPadInputDriver::TestSDLVersion() {
#if SDL_VERSION_ATLEAST(2, 0, 9)
...
void SDLPadInputDriver::QueueControllerUpdate() {
  window_->loop()->Post([]() { SDL_PumpEvents(); });
}
