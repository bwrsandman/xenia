...
class Window {
 public:
  static std::unique_ptr<Window> Create(Loop* loop, const std::u16string& title);
  ...
  virtual bool set_title(const std::u16string& title) {
    if (title == title_) {
      return false;
    }
    title_ = title;
 ...
 protected:
  Window(Loop* loop, const std::u16string& title);
